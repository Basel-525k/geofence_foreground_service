...
class DebugNotificationHelper {
    ...
    func showStartFetchNotification(startDate: Date,
        ...
        DebugNotificationHelper.scheduleNotification(
    ...
    func showCompletedFetchNotification(
        completedDate: Date,
...
//            logInfo("\(logPrefix) \(#function): plugin is not running in debug mode or on iOS 9 or lower")
        ...
        let notificationRequest = createNotificationRequest(
            ...
            threadIdentifier: GeofenceForegroundServicePlugin.identifier,
            title: title,
            body: body,
            icon: icon
        )
    ...
    private static func createNotificationRequest(
        identifier: String,
    ...
    private static var logPrefix: String {
        return "\(String(describing: GeofenceForegroundServicePlugin.self)) - \(DebugNotificationHelper.self)"
    }
}
